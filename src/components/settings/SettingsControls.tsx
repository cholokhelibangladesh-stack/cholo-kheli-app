...
export const SettingsCard = ({
  ...
  <div
    className={`overflow-hidden rounded-2xl border border-border/70 dark:border-white/12 backdrop-blur-xl ${className}`}
    style={{
      background:
        "linear-gradient(180deg, hsl(var(--card) / 0.85) 0%, hsl(var(--card) / 0.55) 100%)",
      boxShadow:
        "inset 0 1px 0 hsl(var(--foreground) / 0.05), 0 10px 28px -22px hsl(var(--teal-deep) / 0.35)",
    }}
  >
    <div className="divide-y divide-border/60 dark:divide-white/[0.07]">{children}</div>
  </div>
);
      ...
      {options.map((o) => {
        ...
        return (
          <button
            ...
            className="flex w-full items-start gap-3 px-4 py-3.5 text-left transition-colors hover:bg-foreground/[0.04] active:bg-foreground/[0.07] dark:hover:bg-white/[0.06] dark:active:bg-white/[0.09]"
          >
            <span
            ...
            >
              {active ? <Check className="h-3.5 w-3.5 text-primary-foreground" strokeWidth={3} /> : null}
            </span>
