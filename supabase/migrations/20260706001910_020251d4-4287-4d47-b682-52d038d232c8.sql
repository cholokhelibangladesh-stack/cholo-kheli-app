...
-- Tighten video read policy to respect profiles.privacy_settings.is_private
DROP POLICY IF EXISTS "Videos readable to authed" ON public.videos;

CREATE POLICY "Videos readable respecting privacy"
ON public.videos
FOR SELECT
TO authenticated
USING (
  ...
  OR NOT EXISTS (
    ...
    WHERE ps.user_id = public.videos.user_id
      AND ps.is_private = true
  )
);

-- Update ranked feed to exclude private-account videos for non-owners
...
BEGIN
  ...
  SELECT ur.role INTO urole FROM public.user_roles ur WHERE ur.user_id = uid LIMIT 1;
  IF urole IS NULL THEN RAISE EXCEPTION 'no_role' USING ERRCODE = '42501'; END IF;

  IF urole = 'scout' THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.scout_profiles sp
      WHERE sp.user_id = uid AND sp.verification_status = 'active' AND COALESCE(sp.is_banned, false) = false
    ) THEN
    ...
    FROM public.scout_profiles sp WHERE sp.user_id = uid;
  END IF;
  ...
  WITH proven AS (
    SELECT
      v.id AS vid, v.user_id AS vuser, v.video_url AS vurl, v.description AS vdesc,
      v.position_tags AS vpos, v.trait_tags AS vtraits,
      COALESCE(v.like_count, 0) AS vlikes, COALESCE(v.share_count, 0) AS vshares,
      COALESCE(v.view_count, 0) AS vviews, COALESCE(v.ranking_score, 0) AS vscore,
      p.full_name AS pname, p.sport AS psport, p.avatar_url AS pavatar,
      false AS is_test,
    ...
    LEFT JOIN public.privacy_settings ps ON ps.user_id = v.user_id
    WHERE v.status = 'live'
      AND COALESCE(v.flagged, false) = false
      AND COALESCE(p.is_banned, false) = false
      AND (urole <> 'player' OR v.user_id <> uid)
      AND (_sport IS NULL OR p.sport = _sport)
      AND (v.user_id = uid OR COALESCE(ps.is_private, false) = false)
  ...
  SELECT m.vid, m.vuser, m.vurl, m.vdesc,
    COALESCE(m.vpos, '{}'), COALESCE(m.vtraits, '{}'),
    COALESCE(m.pname, 'Unknown'), COALESCE(m.psport, 'football'), COALESCE(m.pavatar, ''),
    m.vlikes, m.vshares, m.vviews,
    EXISTS (SELECT 1 FROM public.video_likes vl WHERE vl.video_id = m.vid AND vl.user_id = uid) AS liked_by_me,
    (m.vscore
